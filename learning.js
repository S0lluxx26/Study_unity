...
function renderCourse() {
  ...
  document.querySelector('#lessonView').innerHTML = `
    ...
    <div class="lesson-steps">${m.steps.map(s=>`<div class="lesson-step"><div><h4>${s[0]}</h4><p>${s[1]}</p></div></div>`).join('')}</div>
    <pre class="mini-code"><code></code></pre>
    <div class="checkpoint"><span>PROOF OF LEARNING</span><p>${m.checkpoint}</p></div>
    <div class="lesson-footer"><a href="${m.source}" target="_blank" rel="noreferrer">Read the official Unity reference ↗</a><button class="complete-lesson ${lessonState.has(activeLesson)?'completed':''}" type="button">${lessonState.has(activeLesson)?'Completed ✓':'Mark module complete'}</button></div>`;
